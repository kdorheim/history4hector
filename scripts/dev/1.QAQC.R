...
vars <- c("BC_emissions", "CH4_emissions", "CO_emissions", "ffi_emissions",
          ...
          "OC_emissions", "SO2_emissions", "RF_tot", "RF_CO2", "SV",
          "RF_N2O", "RF_H2O_strat", "RF_O3_trop", "RF_BC", "RF_OC", "RF_CH4",
          "RF_NH3", "RF_SO2", "RF_aci", "global_tas", "O3_concentration", LUC_EMISSIONS(), "N2O_natural_emissions",
          NATURAL_CH4(),
...
# Run old gcam set up
ini <- file.path(BASE_DIR, "data", "dev", "old-hector-gcam.ini")
...
lapply(unique(input_data$variable), function(var){
    ...
    do.call(what = "rbind") %>%
    # we need to exclude the the RF albedo and the SV runs since we will need to
    # those single runs individually.
    filter(!scenario %in% c("RF_albedo", "SV")) ->
    rslts

# Run with the old SV values
hc <- newcore(system.file(package = "hector", "input/hector_ssp245.ini"),
              name = "default")
file.path(BASE_DIR, "data/dev/default_emissions.csv") %>%
    read.csv(comment.char = ";") %>%
    select(Date, SV) ->
    old_sv
setvar(hc, dates = old_sv$Date, var = "SV", values = old_sv$SV, unit = getunits("SV"))
reset(hc)
run(hc)
fetchvars(hc, dates = 1750:2100, vars = c(GLOBAL_TAS(), RF_TOTAL(), RF_VOL(), "SV")) %>%
    mutate(scenario = "SV") ->
    sv_rslts

# Run with the old RF Albedo values
hc <- newcore(system.file(package = "hector", "input/hector_ssp245.ini"),
              name = "default")
file.path(BASE_DIR, "data/dev/default_emissions.csv") %>%
    read.csv(comment.char = ";") %>%
    select(Date, RF_ALBEDO()) ->
    old_albdo
setvar(hc, dates = old_albdo$Date, var = RF_ALBEDO(), values = old_albdo$RF_albedo, unit = getunits(RF_ALBEDO()))
reset(hc)
run(hc)
fetchvars(hc, dates = 1750:2100, vars = c(GLOBAL_TAS(), RF_TOTAL(), RF_ALBEDO())) %>%
    mutate(scenario = RF_ALBEDO()) ->
    albd_rslts

rslts %>%
    rbind(sv_rslts, albd_rslts) ->
    rslts_rslts
...
hc <- newcore(system.file(package = "hector", "input/hector_ssp245.ini"),
             name = "default")
run(hc)
fetchvars(hc, dates = 1750:2100, vars = unique(rslts_rslts$variable)) %>%
...
# The largest changes in the global temp came from the change in the SV forcing!
# Then it is the LUC emissions
